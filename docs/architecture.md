...
**Root identity** is privileged (a root-attributed fs event has no `$PKG` token and surfaces as `$REPO/...`) but the env-derived `npm_package_name` label is forgeable, so on Linux/Firecracker the dispatcher stamps a **non-forgeable** `root_anchored` verdict computed from the kernel process tree plus each pid's cwd at its first `execve` (`isRepoRootAnchored`, `src/guest/root-anchor.ts`) — an event a dependency laundered under the root's name fails the walk and renders `<FORGED_ROOT> $REPO/...` instead of dropping or collapsing. A **nameless root** (a parseable root `package.json` with no `name`) leaves `npm_package_name` empty, so its own lifecycle events would attribute to null; rather than dropping them (or refusing to emit a lockfile), `buildRootPkgKeys` keys it to the synthetic `ROOT_SENTINEL = '<repo-root>'` and the **attribution layer** (`attributionFromEnvVars`'s `rootSentinel` parameter, consumed by both the `/proc` walk and the shim fast-path) attributes the root's own lifecycle events — recognised by an empty `npm_package_name` plus a *canonical* `npm_lifecycle_event` — to the sentinel for *all* event kinds; reads/writes additionally route through the same `root_anchored` stamp. The package-manager driver carries no canonical `npm_lifecycle_event`, so it stays null → dropped (no `<repo-root>` flood). See `docs/design.md` "Non-forgeable root identity".
