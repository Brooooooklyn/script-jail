...
- **A dev-built `script-jail-vm` won't boot VZ until you sign it.** After `cargo build --release -p script-jail-host-mac`, run `codesign --force --sign - --entitlements src/host-mac/script-jail-vm.entitlements target/release/script-jail-vm` — Virtualization.framework refuses to boot without the `com.apple.security.virtualization` entitlement (the CLI preflight reports the same).
